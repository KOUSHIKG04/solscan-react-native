...
export function useWallet() {
    ...
    const connect = useCallback(async () => {
        ...
        try {
            throw new Error(
                "Solana Mobile Wallet Adapter requires a custom Expo development build, not Expo Go."
            );
        } finally {
            setConnecting(false);
        }
    ...
    const sendSOL = useCallback(async () => {
        ...
        try {
            throw new Error(
                "Sending SOL requires Solana Mobile native support in a custom dev build."
            );
        } finally {
            setSending(false);
        }
